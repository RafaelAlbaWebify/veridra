...
def _enforce_origin(request: Request, config: LeadFormConfig) -> None:
    if not config.allowed_origins:
        return
    observed = _request_origin(request)
    if observed not in config.allowed_origins:
...
def _lead_rows(status: LeadStatus | None) -> str:
    rows = "".join(
        "<tr><td><strong>{name}</strong><br>{email}</td><td>{website}</td><td>{status}</td><td>{date}</td><td><a class='button secondary' href='/leads/{identifier}'>Open</a></td></tr>".format(
            name=html.escape(str(lead.name)),
        ...
        for identifier, lead in _leads().list_leads(status=status)
    )
    return rows or "<tr><td colspan='5'>No leads match this view.</td></tr>"
...
async def edit_lead(lead_id: str, request: Request) -> RedirectResponse:
    ...
    try:
        ...
        updated = lead.model_copy(
            update={"status": status, "notes": _single(body, "notes")}
        )
        updated = AuditLead.model_validate(updated)
