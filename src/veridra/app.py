...
def _query_base(
    ...
    base = {"demo": "true"} if demo_mode else {"url": submitted_url}
...
def _profile_label(entry: ProfileEntry) -> str:
    if entry.client_name:
        return f"{entry.organisation_name} — {entry.client_name}"
    return entry.organisation_name
...
def _profile_options(entries: list[ProfileEntry], selected_profile: str | None) -> str:
    ...
    options.extend(
        "<option value='{identifier}'{selected}>{label}</option>".format(
            ...
            selected=" selected" if entry.id == selected_profile else "",
            label=html.escape(_profile_label(entry)),
        )
...
def dashboard(
    ...
    *,
    ...
    entries = profile_entries if profile_entries is not None else ProfileStore().list()
    base = _query_base(submitted_url, demo_mode, selected_profile)
