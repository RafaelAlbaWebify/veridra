...
def _root(request: Request):
    return getattr(request.app.state, "veridra_tenant_data_root", None)
...
def replace_task(
    ...
    _require_project(request, identity, payload.project_id)
    target = _target(identity, task_id)
    try:
        replacement_id = _store(request).replace(identity, target, payload)
