...
def discovery_page(request: Request) -> str:
    ...
    body = f"{navigation}<section><p><a href='/agency/prospects'>← Prospects</a></p><h1>Discover prospects</h1><p class='muted'>Open a bounded Google Maps search in visible Chromium. Nothing is saved until you review the captured businesses and explicitly select them.</p><form method='post' action='/agency/prospects/discover/start'><div class='row'><div><label>Search query</label><input name='query' maxlength='240' value='dentist in Vigo, ES' required></div><div><label>Country code</label><input name='country_code' maxlength='2' value='ES' required></div></div><div class='row'><div><label>Locality</label><input name='locality' maxlength='120' value='Vigo'></div><div><label>Administrative area</label><input name='administrative_area' maxlength='120' value='Pontevedra'></div></div><div class='row three'><div><label>Maximum results</label><input name='max_results' type='number' min='1' max='200' value='20'></div><div><label>Maximum scrolls</label><input name='max_scrolls' type='number' min='0' max='100' value='10'></div><div><label>Maximum seconds</label><input name='max_seconds' type='number' min='1' max='300' value='45'></div></div><button type='submit'>Open discovery browser</button></form></section>"
