...
def _values(body: bytes) -> dict[str, str]:
    ...
    return {name: parsed.get(name, [""])[0].strip() for name in (
        "finding_id", "title", "status", "notes", "owner_label", "due_date", "source_assessment_id"
    )}
...
def _options(selected: TaskStatus) -> str:
    return "".join(
        f"<option value='{item.value}'{' selected' if item == selected else ''}>{html.escape(item.value.replace('_', ' ').title())}</option>"
...
def task_list(project_id: str, status: str | None = Query(default=None)) -> str:
    ...
    rows = "".join(
        "<tr><td><strong>{title}</strong><br><code>{finding}</code></td><td>{status}</td><td>{owner}</td><td>{due}</td><td><div class='actions'><a class='button secondary' href='/projects/{project}/tasks/{identifier}/edit'>Edit</a><a class='button secondary' href='/history/{source}'>Source</a><form class='inline' method='post' action='/projects/{project}/tasks/{identifier}/delete'><button class='danger' type='submit'>Delete</button></form></div></td></tr>".format(
            title=html.escape(task.title), finding=html.escape(task.finding_id), status=html.escape(task.status.value.replace("_", " ")), owner=html.escape(task.owner_label or "Unassigned"), due=html.escape(task.due_date or "Not set"), project=project_id, identifier=identifier, source=task.source_assessment_id
        ) for identifier, task in entries
    ) or "<tr><td colspan='5'>No remediation tasks match this view.</td></tr>"
    filters = " ".join(["<a class='button secondary' href='?'>All</a>"] + [f"<a class='button secondary' href='?status={item.value}'>{html.escape(item.value.replace('_', ' ').title())}</a>" for item in TaskStatus])
    body = f"<section><h1>{html.escape(project.name)} remediation tasks</h1><p class='muted'>Statuses are explicit operator decisions; Veridra does not mark work fixed or verified automatically.</p><div class='actions'>{filters}<a class='button' href='/projects/{project_id}/monitor'>Project monitoring</a></div></section><section><table><thead><tr><th>Task / finding</th><th>Status</th><th>Owner</th><th>Due</th><th>Actions</th></tr></thead><tbody>{rows}</tbody></table></section>"
...
def new_task(project_id: str, assessment: str, finding: str) -> str:
    ...
    task = RemediationTask(project_id=project_id, finding_id=match.id, title=match.title, source_assessment_id=assessment)
    return _page("Create remediation task", _form(project_id, task, action=f"/projects/{project_id}/tasks", heading="Create remediation task"))
...
def edit_task(project_id: str, task_id: str) -> str:
    ...
    return _page("Edit remediation task", _form(project_id, task, action=f"/projects/{project_id}/tasks/{task_id}/edit", heading="Edit remediation task"))
